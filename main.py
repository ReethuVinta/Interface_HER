...
batch_size = 4
device = torch.device('cpu')
model = Nets(batch_size,use_gpu).to(device)
model.load_state_dict(torch.load('./models/model_epoch30', map_location=device))

# video_model = Nets(batch_size,use_gpu).to(device)
# video_model=torch.load('./models/model_epoch30',map_location=device)
# video_model.eval()

# model.eval()
# print(Xmodel.summary())
#####################################################################
def GetVideoFeatures(Path):
	cap = None
...
#####################################################
def video_analysis(video_path,final_model,feature_model):
    video_features=GetVideoFeatures(video_path)
    predictions=final_model(video_features)
    return predictions
#####################################################
...
def video():
    if request.method == 'POST':
        ...
        f.save(file_path)
        predictions = video_analysis(file_path,video_model,Xmodel)
